...
const EditPrasarana = () => {
  ...
  const handleRemoveItem = (index) => {
    if (prasarana.length > 1) {
      setPrasarana(prasarana.filter((_, i) => i !== index));
    } else {
      showNotification('error', 'Minimal harus ada 1 kategori prasarana.');
    }
  };
    ...
    >
      ...
      >
        ...
        <p className="text-sm text-gray-600 mt-2">
          Anda dapat menambah kategori baru, mengubah fasilitas, atau menghapus kategori yang tidak diperlukan.
        </p>
      </motion.div>
      ...
      >
        ...
        >
          {prasarana.map((item, index) => (
            <motion.div 
              key={index} 
              className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-neutral/50 rounded-lg border relative"
              variants={itemVariants}
